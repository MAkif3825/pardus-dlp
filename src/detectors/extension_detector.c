...
static int
extension_init(void *config)
{
	if (!config)
		return -1;

	// Cast the generic configuration block to our custom extension
	// configuration layout
	extension_config_t *ctx = (extension_config_t *)config;
	if (!ctx->ext_map || !ctx->db_path)
		return -1;

	// Fire our core loader to push the extensions.txt data straight into
	// the kernel map register
...
extension_handle(const struct dlp_event *e, char *reason, size_t rlen)
{
	if (!e)
	...
	if (e->flags == 0xDEAD) {
		// Find the extension in the path string to generate a clean
		// alert message
		...
		if (ext != NULL) {
			...
			snprintf(
			    ...
			    "hidden as a data "
			    "document [Extension: .%s, Process: %s, Path: %s]",
			    ext, e->comm, e->full_path);

			return DETECTION_ALERT; // Trigger an explicit alert
						// flag to your main loop!
		}
	}
