...
SEC("lsm/file_open")
int
BPF_PROG(dlp_file_open, struct file *file)
{
	...
	if (path_len < 0) {
		// The path is invalid or can't be resolved. Release the memory
		// block.
...
int
BPF_PROG(dlp_bprm_check, struct linux_binprm *bprm)
{
	__u64 pid_tgid = bpf_get_current_pid_tgid();
	...
	if (ext_idx > 0 && ext_idx < 128) {
		...
		if (allowed != NULL) {
			// Allocate an event payload inside your lockless ring
			// buffer 'rb'
			struct dlp_event *e
			...
			if (e) {
				...
				e->flags = 0xDEAD; // Custom flag telling
						   // userspace it was blocked

				bpf_get_current_comm(&e->comm,
						     sizeof(e->comm));
				bpf_probe_read_kernel_str(e->full_path,
							  sizeof(e->full_path),
							  filename);
