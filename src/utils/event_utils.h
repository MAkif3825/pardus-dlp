 ...
 * namespaces to resolve relative file references back into static canonical system strings.
 *
 * @param raw Pointer to the incoming telemetry packet from the kernel map.
 * @param out Pointer to the enriched tracking container data structure to populate.
 * @return 0 on success, or -1 if canonicalization fails and falls back to raw data.
 */
