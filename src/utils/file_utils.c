			    ...
			    size_t max_size_bytes)
{
	...
	// 🛡️ GUARDRAIL 1: Query file metadata directly from OS inode (Does not
	// read contents)
	struct stat st;
	if (stat(filepath, &st) != 0)
		return false; // File doesn't exist or permissions denied

	// Ensure we are dealing with a regular file, skipping directories,
	// pipes, and sockets
	...
	// Convert the raw binary bytes into a clean 64-character hexadecimal
	// string
...
trim_whitespace(char *str)
{
	...
	// 3. Drop a clean Null Terminator directly into memory right after the
	// last character
	*(end + 1) = '\0';
}